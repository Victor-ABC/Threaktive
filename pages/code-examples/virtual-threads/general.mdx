...
Die Funktionsweise der Java Virtual Machine (JVM) wird durch die virtuellen Threads grundlegend verändert,
aber die Integration ist sehr ähnlich, zu den Platform (normalen) Threads.
...
zu Erstellen oder einen `Executor` zu verwenden. Es besteht auch die Option, im gesamten Projekt das Aktivieren
virtueller Threads im Spring Boot Framework zu nutzen.
...
Ein `ExecutorService` ist eine Java-Schnittstelle, die es ermöglicht, Aufgaben zur Ausführung an Threads zu delegieren.
Der `ExecutorService` verwaltet einen Pool von Threads und nimmt Aufgaben entgegen, die dann von dem Thread Pool ausgeführt warden.
Ein `ExecutorService` ist bei der Verwendung von Platform Threads weit verbreitet.
...
Zuerst wird ein `ExecutorService` mit Platform Threads erstellt, hier muss die Anzahl der Platform Threads
