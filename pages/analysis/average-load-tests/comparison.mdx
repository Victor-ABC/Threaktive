...
Um das Verhältnis zwischen den beiden Szenarien zu verstehen, betrachten wir die
Verhältnisse der jeweiligen Werte:

### Varianz

```
 Varianz VirtualThreads     860693.4741225742
------------------------ = ------------------- ≈ 1.355
 Varianz normal             635313.9680897696
```

Das Verhältnis beträgt etwa 1,355. Das bedeutet, dass die Varianz in "VirtualThreads" etwa
1,355-mal größer ist als die Varianz im "normalen" Szenario.

### Standardabweichung

```
 Standardabweichung VirtualThreads     927.475
----------------------------------- = --------- ≈ 1.163
 Standardabweichung normal	           796.440
```

Das Verhältnis beträgt etwa 1,163. Das bedeutet, dass die Standardabweichung in "VirtualThreads"
etwa 1,163-mal größer ist als die Standardabweichung im "normalen" Szenario. In beiden Fällen
ist das Verhältnis größer als 1, was darauf hinweist, dass die Streuung in "VirtualThreads"
im Vergleich zu "normal" größer ist, sei es in Bezug auf Varianz oder Standardabweichung.
...
bei der normalen Implementierung. Diese Unterschiede werden auch durch die Analyse der Perzentile
...
normalen Implementierung. Diese Werte zeigen, dass die Virtual Threads in einigen Fällen
...
zwischen den Implementierungen (normal und Virtual Threads) sind weniger ausgeprägt. Die
...
(5,28 bzw. 8,8 Millisekunden) sind vergleichbar mit den Werten der normalen Implementierung
...
durchschnittliche Antwortzeiten bieten können. Jedoch sollte die Entscheidung zwischen normalen
